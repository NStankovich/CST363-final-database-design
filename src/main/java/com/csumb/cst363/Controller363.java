...
public class Controller363 {
   ...
   public String addPrescription(@Valid Prescription p, BindingResult result, Model model) {
      ...
      try {
         ...
         if(addPrescriptionValidation(p, conn)) {
            ...
            //Inserts values of doctorid, patientid, drugid and localdate object into database
            ps = conn.prepareStatement("INSERT INTO prescription(doctorid,patientid,drugid,prescribeddate,quantity) VALUES (?,?,?,?,?)");
   ...
   /**
    * Private helper function for addPrescription method. Performs input
    * validation on Patient SSN/Name, Doctor SSN/Name and the drug name.
   ...
   private boolean addPrescriptionValidation(Prescription p, Connection conn) {
      try {
         ...
         //Return false if provided patient name does not match name associated with provided SSN
         else if (!(rs.getString(1) + " " + rs.getString(2)).toLowerCase().equals(p.getPatientName().toLowerCase()) ){
         ...
         //Return false if provided doctor name does not match name associated with provided SSN
   ...
   public String processFillForm(@Valid Prescription p, BindingResult result, Model model) {
      ...
      try {
         ...
         if(fillPrescriptionValidation(p,conn)) {
            ...
            PreparedStatement ps = conn.prepareStatement(""
                  + "SELECT do.firstname, do.lastname, pr.quantity "
                  + "FROM prescription pr "
                  + "INNER JOIN patient pa ON pr.patientid = pa.patientid "
                  + "INNER JOIN doctor do ON pr.doctorid = do.doctorid "
            ...
            if(!rs.next()) {
               ps = conn.prepareStatement(""
                     + "INSERT INTO fills(prescriptionid, pharmacyid, datefilled) "
                     + "VALUES(?,?,?)");
               ps.setInt(1, Integer.parseInt(p.getRxid()));
            ...
            if(insertedRows != 1) {
               ...
               ps = conn.prepareStatement("WITH u(prescriptionid, pharmacyid, drugid, quantity, datefilled) AS ( "
                     ...
                     + "FROM fills f "
                     + "INNER JOIN prescription pr ON f.prescriptionid = pr.prescriptionid "
                     + "WHERE f.prescriptionid = ? ) "
                     + "SELECT u.quantity, ph.phonenumber, u.datefilled, dr.tradename, u.quantity*s.pharmacyprice "
                     + "FROM sells s "
                     + "INNER JOIN u ON s.pharmacyid = u.pharmacyid AND s.drugid = u.drugid "
   ...
   /**
    * Private helper method for processFillForm. Performs input validation
    * on Rxid, PharmacyName and PharmacyAddress.
    * @param p Prescription object to be updated
    * @param conn Connection to MySQL database
   ...
   private boolean fillPrescriptionValidation(Prescription p, Connection conn) {
      ...
      //word
      if(p.getPatientName().indexOf(" ") == -1 
            || p.getRxid().equals("")
            || p.getPharmacyAddress().equals("") 
            || p.getPharmacyName().equals("")) {
         p.setRxid("Error: Form cannot have empty values and patient name must contain first and last name separated by a space.");
         return false;
      }
      ...
      try {
         ...
         //returns false if result is 0 (no records found) or >1 (not unique)
               ...
               || Integer.parseInt(rs.getString(1)) > 1){
            p.setRxid("Error: No results found for this patient and prescription.");
            return false;
         }
         ...
         //name and address
         ps = conn.prepareStatement(""
               + "SELECT COUNT(pharmacyid), pharmacyid "
               + "FROM pharmacy "
   ...
   /**
    * Private helper method for fillPrescriptionValidation. Checks to see if
    * the provided pharmacy sells the provided drug id.
   ...
   private boolean doesPharmacySellDrug(Prescription p, Connection conn) {
      try {
         ...
         PreparedStatement ps = conn.prepareStatement(""
               + "WITH u(prescriptionid, drugid, firstname, lastname) AS ( "
               + "SELECT pr.prescriptionid, pr.drugid, pa.firstname, pa.lastname "
               + "FROM prescription pr "
               + "INNER JOIN patient pa ON pr.patientid = pa.patientid "
               + "WHERE pr.prescriptionid = ?) "
               + "SELECT * "
               + "FROM sells s "
               + "INNER JOIN u ON u.drugid = s.drugid "
               + "INNER JOIN pharmacy ph on s.pharmacyid = ph.pharmacyid "
               + "WHERE ph.pharmacyid = ?");
         ps.setString(1, p.getRxid());
   ...
   public String pharmacyReport(
      ...
      try {
         ...
         if(!rs.next()) {
            ...
            ps =  conn.prepareStatement(""
                  + "SELECT dr.tradename, sum(quantity) "
                  + "FROM prescription pr "
                  ...
                  + "WHERE p.pharmacyid = ? AND f.datefilled BETWEEN ? AND ? "
                  + "AND dr.tradename LIKE ?"
   ...
   public String fdaReport(
      ...
      try {
         ...
         //drug name
         PreparedStatement ps = conn.prepareStatement(""
               + "SELECT COUNT(DISTINCT tradename), tradename "
               + "FROM drug "
         ...
         if(rs.next() && Integer.parseInt(rs.getString(1)) == 1) {
            ...
            ps = conn.prepareStatement(""
                  + "SELECT do.doctorid, do.firstname, do.lastname, SUM(pr.quantity) "
                  + "FROM prescription pr "
                  + "INNER JOIN doctor do ON pr.doctorid = do.doctorid "
                  + "INNER JOIN drug dr ON pr.drugid = dr.drugid "
                  + "WHERE dr.tradename LIKE ? "
                  + "AND pr.prescribeddate BETWEEN ? AND ? "
                  + "GROUP BY do.doctorid");
            ps.setString(1, drug+"%");
            ps.setString(2, startDate);
            ps.setString(3, endDate);
            rs = ps.executeQuery();
